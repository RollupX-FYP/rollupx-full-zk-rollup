...
for index, exp in enumerate(experiments_to_run, start=1):
    ...
    for i, seed in enumerate(seeds[:repeats], start=1):
        ...
        env.update({
            ...
            "ADAPTIVE_SMALL_TIMEOUT_MS": str(exp.get("adaptive_small_timeout_ms", baseline.get("adaptive_small_timeout_ms", 500))),
            "ADAPTIVE_MEDIUM_TIMEOUT_MS": str(exp.get("adaptive_medium_timeout_ms", baseline.get("adaptive_medium_timeout_ms", 1000))),
            "ADAPTIVE_LARGE_TIMEOUT_MS": str(exp.get("adaptive_large_timeout_ms", baseline.get("adaptive_large_timeout_ms", 2000))),
        ...
        if dry_run:
            selected_env = " ".join(
                f"{k}={env[k]}" for k in (
                    ...
                    "ADAPTIVE_SMALL_TIMEOUT_MS", "ADAPTIVE_MEDIUM_TIMEOUT_MS",
                    "ADAPTIVE_LARGE_TIMEOUT_MS",
