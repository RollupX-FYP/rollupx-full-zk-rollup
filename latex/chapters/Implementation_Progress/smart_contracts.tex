...
\begin{itemize}
    \item \textbf{Modular Verification}: The bridge maps \texttt{verifierId} values to \texttt{IVerifier} implementations, enabling multiple proof systems to be selected without changing the bridge logic:
    \begin{itemize}
        \item \texttt{verifierId=0}: \textbf{Groth16} (real SNARK verification on BN254).
        \item \texttt{verifierId=1}: \textbf{Plonky2} (benchmarking/simulation).
        \item \texttt{verifierId=2}: \textbf{Halo2} (benchmarking/simulation).
    \end{itemize}
    \item \textbf{Split DA Commitment}: To satisfy BN254 scalar field constraints in Groth16 public inputs, the 256-bit DA commitment (Keccak hash) is explicitly split into two 128-bit chunks (\texttt{daLow}, \texttt{daHigh}). This ensures the inputs fit within the field modulus ($p \approx 254$ bits), preserving cryptographic binding while remaining circuit-compatible.
...
Decoupling the proof verification logic from the bridge (via the \texttt{IVerifier} interface) allows for upgrading the proof system (e.g., from Groth16 to Plonky2) without redeploying the main bridge contract. This is essential for a research testbed where cryptographic primitives are subject to change.

\paragraph{Cryptographic Compatibility (Groth16)}
A critical implementation detail involves the G2 point representation. The Rust-based prover (Arkworks) outputs G2 points as $(c0, c1)$ (imaginary, real), whereas the Ethereum `ecPairing` precompile expects $(c1, c0)$ (real, imaginary). The bridge logic explicitly handles this coordinate swapping in the `commitBatch` function to ensure valid proof verification on-chain, bridging the gap between the off-chain ZK circuit and the EVM.
