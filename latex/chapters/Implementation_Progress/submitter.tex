...
Implemented in \texttt{infrastructure/da\_offchain.rs}, this strategy models storing the batch data externally and posts only a compact \textbf{data commitment} (CID/hash) to the L1 contract. This achieves minimal on-chain data cost but introduces an explicit availability trust assumption on the external data layer. The current off-chain DA mode is a local or simulated storage stub intended to model future external DA integration. IPFS, Celestia, and Avail are future-work targets, not completed integrations.
