...
The \textbf{prototype's proving module} is designed to integrate proving pathways for a \textbf{zk-Rollup (Zero-Knowledge Rollup)} scaling solution. Its primary purpose in the context of benchmarking is to model the computational overhead of cryptographic validity proofs and isolate data availability and sequencer bottlenecks.

The prototype uses a modular proving interface that supports a parameterized mock prover for benchmarking and a RISC Zero proving path where applicable. The mock prover is used to isolate sequencer, executor, and DA bottlenecks without treating proof generation as a fixed implementation dependency.

\subsubsection{System Components (Broad View)}

The system consists of several components that interact to form the full rollup pipeline:

\paragraph{Rollup Smart Contract (L1  Solidity)}

Located in the \texttt{contracts} directory, this is the on-chain anchor. It:
\begin{itemize}
    \item Keeps track of the \textbf{state root} (a single 32-byte hash representing the entire L2 account tree).
    \item Supports pluggable verifier stubs to evaluate bridge and DA mechanisms.
\end{itemize}
