...
At a code level, the Submitter performs four main responsibilities. First, it loads the relevant submission configuration, including the selected data availability mode, bridge contract address, network endpoint, and retry or confirmation parameters. Second, it prepares the batch payload by selecting the appropriate data availability implementation, such as calldata mode, blob-style mode, or off-chain data availability mode. Third, it invokes the Layer-1 bridge client to submit the batch commitment, data availability metadata, state root, and proof-related bytes to the settlement contract. Finally, it waits for the transaction receipt, verifies the execution status, and records metrics such as submission latency, gas usage, and confirmation result.
...
The core idea of the abstraction is shown below:
...
The Submitter supports multiple data availability strategies so that the benchmark suite can compare different L1 data publication models under a consistent pipeline. My contribution included implementing and refactoring the concrete DA strategy modules used by the Submitter.
