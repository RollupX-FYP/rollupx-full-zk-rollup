...
\section{Engineering Workload Summary}

Table \ref{tab:engineering_workload} summarizes my core technical contribution areas, confirming that the workload meets the rigor and scale of an individual Final Year Project.

\begin{table}[h!]
\centering
\caption{Engineering Workload Summary}
\label{tab:engineering_workload}

\begin{tabularx}{\textwidth}{|p{0.25\textwidth}|X|X|p{0.20\textwidth}|}
\hline
\textbf{Contribution Area} & \textbf{Main Code Written} & \textbf{Technical Complexity} & \textbf{Evidence} \\ \hline
Submitter daemon & \texttt{orchestrator.rs}, \texttt{ethereum\_adapter.rs}, DA strategies & Managed async L1 polling, mockable trait abstraction, receipt status checks & Git commits, Rust source files \\ \hline
L1 bridge/DA/verifier & \texttt{ZKRollupBridge.sol}, \texttt{BlobDA.sol}, \texttt{MockVerifier.sol} & Solidity state root management, DA commitment verification, mock validation & Solidity source, Hardhat deploy scripts \\ \hline
Benchmark orchestration scripts & \texttt{run\_experiment.sh}, \texttt{run\_matrix.sh}, \texttt{experiments.toml} & Docker lifecycle management, systematic TOML parsing, sequencer readiness polling & Bash scripts, TOML configs \\ \hline
Poisson workload generator & \texttt{poisson\_generator.py} & Exponential inter-arrival delays for burst simulations, seeded RNG & Python workload logic \\ \hline
Data aggregation & \texttt{aggregate.py}, \texttt{stats.py}, plotting logic & Cross-service JSONL joining by batch ID, percentile computation, Pareto plots & Python tools \\ \hline
Integration and bug fixes & \texttt{service.rs} parsing, batch policy configs & Fixed hidden upstream panics (\texttt{strip\_prefix}) and TOML policy defaults & Executor/Sequencer Git history \\ \hline
\end{tabularx}
