...
\newpage
\noindent
Exact excerpt from \path{contracts/contracts/bridge/ZKRollupBridge.sol}:
...
The technical value of this design is that the Submitter can call one stable contract interface while the benchmark framework changes the DA or verifier configuration. This makes the L1 settlement path modular. Instead of hardcoding one DA mechanism or one verifier, the bridge uses identifiers such as \texttt{daId} and \texttt{verifierId} to select the appropriate contract-side logic. Table \ref{tab:contract_components} details these component responsibilities.

\begin{table}[!htbp]
\centering
\caption{Contract Component Responsibilities}
\label{tab:contract_components}
\small
...
>{\raggedright\arraybackslash}X|}
\hline
\textbf{Contract Component} & \textbf{Responsibility} & \textbf{Connected Submitter Data} \\ \hline
\texttt{commitBatch} Router & Parses incoming L1 tx and routes to appropriate DA/verifier. & Core arguments (DA/Verifier IDs). \\ \hline
DA Provider (\texttt{IDAProvider}) & Validates commitment and extracts metadata. & \texttt{batchData} and \texttt{daMeta}. \\ \hline
Verifier (\texttt{IVerifier}) & Validates the state transition via zk-SNARK or mock validation. & \texttt{newRoot}, DA commitment, \texttt{proof}. \\ \hline
State Root Updater & Finalizes the batch and emits \texttt{BatchCommitted}. & \texttt{newRoot}. \\ \hline
\end{tabularx}
\end{table}

To connect the Submitter to this contract interface, I implemented the \texttt{BridgeClient} trait in the Submitter service, which maps directly to the \texttt{commitBatch} contract signature. This abstraction provides a testable interface for Layer-1 interactions.

\noindent
Exact excerpt from \path{submitter/src/infrastructure/ethereum_adapter.rs}:

\begin{lstlisting}[language=Rust, numbers=left, breaklines=true, basicstyle=\ttfamily\small]
#[async_trait]
...
Although the benchmark environment is local and controlled, keeping the implementation closer to realistic smart contract engineering practices prevents testing edge cases from silently hiding bugs.
...
The resulting integration path follows the settlement architecture outlined previously. The Submitter acts as the single point of entry from the Layer-2 system, passing the finalized payload to the \texttt{ZKRollupBridge}. The bridge then orchestrates the validation of the data availability mode and the proof verification before updating the state root.
