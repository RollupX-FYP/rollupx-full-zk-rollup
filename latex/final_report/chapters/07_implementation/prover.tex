...
\begin{itemize}
    ...
    \item \textbf{Real Prover Backend:} A \texttt{risc0\_prover} implementation exists and is used as the real proof path backend for the executor (\texttt{PROVER\_BACKEND=risc0}). When enabled via \texttt{REQUIRE\_REAL\_PROOFS}, it executes the guest code inside the RISC0 zkVM, generating STARK proofs that can be wrapped in Groth16.
