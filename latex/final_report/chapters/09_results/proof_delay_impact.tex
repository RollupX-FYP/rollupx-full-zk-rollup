...
T(N) = A + B \cdot N
\end{equation}
where $A$ represents the fixed compiler loading, setup, and SNARK-wrapping overhead (observed consistently at \textbf{70 to 80 seconds}), and $B$ represents the marginal proving cost (modeled at \textbf{~5.7 seconds per transaction}).
...
Evaluating the zkVM guest logs reveals that each L2 transaction consumes between \textbf{41,000 and 45,000 cycles} depending on the state-access footprint. RISC0 executes and proves guest code in discrete segments of exactly $2^{20}$ ($1,048,576$) cycles. Because proving is performed segment-by-segment, the wall-clock proving time scales in a step-function governed by these segment boundaries, with each additional segment adding approximately \textbf{140 to 150 seconds}:
\begin{itemize}
