...
use std::io::Write;
use serde::Serialize;
use crate::domain::cost_breakdown::CostBreakdown;
...
struct SubmitterMetrics {
    ...
    da_posting_blob_gas_estimate: u64,
    overhead_gas_estimate: u64,
    proof_verify_pct: f64,
    da_pct: f64,
    overhead_pct: f64,
    ...
    final_gas_price_gwei: Option<f64>,
}
                ...
                match da_strategy.submit(&batch, &proof_hex, verifier_id).await {
                    Ok(result) => {
                        ...
                        if let Ok(Some(rec)) = outbox.get_record(&fetched.batch_id) {
                            gas_bump_count = rec.gas_bump_count as u8;
                    ...
                    Err(e) => {
                        ...
                        let metrics = SubmitterMetrics {
                            ...
                            da_posting_blob_gas_estimate: 0,
                            overhead_gas_estimate: 0,
                            proof_verify_pct: 0.0,
                            da_pct: 0.0,
                            overhead_pct: 0.0,
