...
sudo apt install -y \
  ...
  libssl-dev sqlite3 python3-venv jq curl gettext-base
```
...
# Quick health check
curl -sf http://127.0.0.1:3000/health && echo "Sequencer: OK" || echo "Sequencer: DOWN"
...
4. Templates `config/sequencer.template.toml` → writes a per-run sequencer config
5. Optionally restarts the sequencer binary with the new config
...
| Sequencer `/health` returns connection refused | Sequencer failed to start | Check sequencer terminal for error; check port conflicts |
